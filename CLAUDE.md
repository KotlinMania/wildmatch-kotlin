# Claude Code Project Instructions — wildmatch-kotlin

## Project Overview

This is **wildmatch-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`wildmatch`](https://crates.io/crates/wildmatch).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/wildmatch/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/wildmatch>
