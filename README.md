...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`wildmatch`](https://crates.io/crates/wildmatch).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `wildmatch` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `wildmatch` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
